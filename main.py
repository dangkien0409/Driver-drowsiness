...
Sử dụng camera để theo dõi mắt và ngáp để phát hiện ngủ gật
"""
...
class DrowsinessDetectionSystem:
    ...
    def process_frame(self, frame):
        ...
        # Add status info
        status_text = "NORMAL" if not confirmed_drowsy else "DROWSY!"
        color = (0, 255, 0) if not confirmed_drowsy else (0, 0, 255)
        
        cv2.putText(frame_with_results, f"Status: {status_text}", (10, 90),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2)
        cv2.putText(frame_with_results, f"Frames: {self.drowsy_frame_count}/{self.consecutive_frames}", (10, 120),
        ...
        cv2.putText(frame_with_results, f"Frame: {self.frame_count}", (10, 170),
